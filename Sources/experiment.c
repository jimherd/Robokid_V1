...
uint8_t   i, j, ad_value;
    ...
    for(i=0 ; i<count ; i++){
        for(j=0 ; j<100 ; j++) {
            set_motor(LEFT_MOTOR, M_FORWARD, 75);
            set_motor(RIGHT_MOTOR, M_FORWARD, 75);
            DelayMs(30000);
            ...
            send_msg(bcd(ad_value, tempstring));
            send_msg(",\r\n");                                           
            set_motor(LEFT_MOTOR, M_BACKWARD, 50);
            set_motor(RIGHT_MOTOR, M_BACKWARD, 50);
            DelayMs(30000);
            ad_value = get_adc(BATTERY_VOLTS);                     
            send_msg(bcd(ad_value, tempstring));
            send_msg(",");
...
uint8_t   i, j;
    ...
    for(i=0 ; i<count ; i++){
        for (j= 100 ; j>0 ; j -=10) {
            ...
            set_motor(LEFT_MOTOR, M_FORWARD, j);
            set_motor(RIGHT_MOTOR, M_FORWARD, j);
            DelayMs(5000);
        }    
    }
    return 0;
