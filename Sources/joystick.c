...
//        
        ...
        if (ad_value < (BACKWARD_VALUE + DEADBAND)) {    
            L_state = M_BACKWARD;    // backward
        } else if (ad_value > (STOP_VALUE - DEADBAND)) {
            L_state = M_BRAKE;                // stop
        } else {                                 
            L_state = M_FORWARD;      // forward
        }  
//
...
//
        ...
        if (ad_value < (BACKWARD_VALUE + DEADBAND)) {    
            R_state = M_BACKWARD;   // backward              
        } else if (ad_value > (STOP_VALUE - DEADBAND)) {        
            R_state = M_BRAKE;                // stop
        } else {                                 
            R_state = M_FORWARD;    // forward
        } 

//
// configure motor controllers
//
		if ((L_state == M_FORWARD) && (R_state == M_FORWARD)) {
			set_motor(LEFT_MOTOR, M_FORWARD, DEFAULT_SPEED);
			set_motor(RIGHT_MOTOR, M_FORWARD, DEFAULT_SPEED);
		}
		if ((L_state == M_BACKWARD) && (R_state == M_BACKWARD)) {
			set_motor(LEFT_MOTOR, M_BACKWARD, DEFAULT_SPEED);
			set_motor(RIGHT_MOTOR, M_BACKWARD, DEFAULT_SPEED);
		}
		if ((L_state == M_FORWARD) && (R_state == M_BACKWARD)) {
			set_motor(LEFT_MOTOR, M_FORWARD, SPIN_SPEED);
			set_motor(RIGHT_MOTOR, M_BACKWARD, SPIN_SPEED);
		}
		if ((L_state == M_BACKWARD) && (R_state == M_FORWARD)) {
			set_motor(LEFT_MOTOR, M_BACKWARD, SPIN_SPEED);
			set_motor(RIGHT_MOTOR, M_FORWARD, SPIN_SPEED);
		}
		if ((L_state == M_BRAKE) && (R_state == M_BRAKE)) {
			set_motor(LEFT_MOTOR, M_BRAKE, 0);
			set_motor(RIGHT_MOTOR, M_BRAKE, 0);
		}
		if ((L_state == M_FORWARD) && (R_state == M_BRAKE)) {
			set_motor(LEFT_MOTOR, M_FORWARD, 30);
			set_motor(RIGHT_MOTOR, M_BRAKE, 0);
		}
			if ((L_state == M_BRAKE) && (R_state == M_FORWARD)) {
			set_motor(LEFT_MOTOR, M_BRAKE, 0);
			set_motor(RIGHT_MOTOR, M_FORWARD, 30);
		}
	}
}
...
//
        ad_value = get_adc(PAD_SWL);
        if (ad_value < (BACKWARD_VALUE + DEADBAND)) {    
            set_motor(LEFT_MOTOR, M_BACKWARD, left_speed);     // backward
        } else if (ad_value > (STOP_VALUE - DEADBAND)) {
            set_motor(LEFT_MOTOR, M_BRAKE, 0);                 // stop
        } else {                                 
            set_motor(LEFT_MOTOR, M_FORWARD, left_speed);      // forward
        }   
//
// read and process right input switch
//
        ad_value = get_adc(PAD_SWR);
        if (ad_value < (BACKWARD_VALUE + DEADBAND)) {    
            set_motor(RIGHT_MOTOR, M_BACKWARD, right_speed);   // backward              
        } else if (ad_value > (STOP_VALUE - DEADBAND)) {        
            set_motor(RIGHT_MOTOR, M_BRAKE, 0);                // stop
        } else {                                 
            set_motor(RIGHT_MOTOR, M_FORWARD, right_speed);    // forward
        } 
...
//
        ...
        if (ad_value < (CORNER_MODE_STOP_VALUE + DEADBAND)) {    
            set_motor(LEFT_MOTOR, M_OFF, 0);                              // stop
        } else if (ad_value > (CORNER_MODE_HALF_SPEED_VALUE - DEADBAND)) {
            set_motor(LEFT_MOTOR, M_FORWARD, (left_speed - 20));          // half speed
        } else {
            set_motor(LEFT_MOTOR, M_FORWARD, left_speed);                 // full speed
        }   
//
// read and process right input switch
//
        ad_value = get_adc(PAD_SWR);   
        if (ad_value < (CORNER_MODE_STOP_VALUE + DEADBAND)) {    
            set_motor(RIGHT_MOTOR, M_OFF, 0);                             // stop
        } else if (ad_value > (CORNER_MODE_HALF_SPEED_VALUE - DEADBAND)) {
            set_motor(RIGHT_MOTOR, M_FORWARD, (right_speed - 20));        // half speed      
        } else {
            set_motor(RIGHT_MOTOR, M_FORWARD, right_speed);               // full speed
        } 
    }
}
...
//
        switch (joystick_mode) {
            case 0 :  // veer to right
                set_motor(LEFT_MOTOR, M_FORWARD, left_speed);
                set_motor(RIGHT_MOTOR, M_BRAKE, 0);
                break;
            case 1 :  // move forward
                set_motor(LEFT_MOTOR, M_FORWARD, left_speed);
                set_motor(RIGHT_MOTOR, M_FORWARD, right_speed);
                break;        
            case 2 :  // veer to left
                set_motor(LEFT_MOTOR, M_BRAKE, 0);
                set_motor(RIGHT_MOTOR, M_FORWARD, right_speed);
            ...
            case 4 :  // spin right
                set_motor(LEFT_MOTOR, M_FORWARD, left_speed);
                set_motor(RIGHT_MOTOR, M_BACKWARD, right_speed);
                break;            
            case 5 :  // stop
                set_motor(LEFT_MOTOR, M_BRAKE, 0);
                set_motor(RIGHT_MOTOR, M_BRAKE, 0);
                break;           
            case 6 :  // spin left
                set_motor(LEFT_MOTOR, M_BACKWARD, left_speed);
                set_motor(RIGHT_MOTOR, M_FORWARD, right_speed);
                break;           
