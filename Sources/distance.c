...
//
    set_motor(LEFT_MOTOR, M_FORWARD, WHEEL_SENSOR_CALIBRATE_SPEED);
    set_motor(RIGHT_MOTOR, M_FORWARD, WHEEL_SENSOR_CALIBRATE_SPEED);
    DelayMs(2000);
    vehicle_stop();
...
//    
    if (l_speed < 0) {
        set_motor(LEFT_MOTOR, M_BACKWARD, (uint8_t)(-l_speed));
    } else {
        set_motor(LEFT_MOTOR, M_FORWARD, (uint8_t)l_speed); 
    }
//
// set right motor; check if forward or backward move required
// 
    if (r_speed < 0) {
        set_motor(RIGHT_MOTOR, M_BACKWARD, (uint8_t)(-r_speed));
    } else {
        set_motor(RIGHT_MOTOR, M_FORWARD, (uint8_t)r_speed); 
    }
//
