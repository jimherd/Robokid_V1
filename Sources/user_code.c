...
#pragma INTO_ROM
seven_seg_display_t  Press_A = { 
    SEVEN_SEG_AB, 9, 0, 80, 0,
    CHAR_P, CHAR_r, CHAR_E, CHAR_S, CHAR_S, CHAR__, CHAR_A, CHAR_CLR, CHAR_CLR, 0,0,0,0,0,0,0,
    CHAR_CLR, CHAR_P, CHAR_r, CHAR_E, CHAR_S, CHAR_S, CHAR__, CHAR_A, CHAR_CLR, 0,0,0,0,0,0,0,
};
#pragma INTO_ROM
...
void user_init(void) {
    ...
    state_of_vehicle = V_STOPPED;
    left_motor_state = M_OFF;
    right_motor_state = M_OFF;
    ...
    set_motor(RIGHT_MOTOR, M_OFF, 0);
    set_motor(LEFT_MOTOR , M_OFF, 0);
...
//      -              V_STOPPED, V_FORWARD, V_BACKWARD, V_FORWARD_TURN_LEFT, V_FORWARD_TURN_RIGHT
//  7-seg didplay         0           1                 2              3            4
// 
void set_vehicle_state(void) {

    if ((left_motor_state == M_FORWARD) && (right_motor_state == M_FORWARD)) { 
        state_of_vehicle = V_FORWARD;
        current_speed = straight_line_speed;
    } else if ((left_motor_state == M_BACKWARD) && (right_motor_state == M_BACKWARD)) {
        state_of_vehicle = V_BACKWARD;
        current_speed = straight_line_speed;
    } else if ((left_motor_state == M_BACKWARD) && (right_motor_state == M_FORWARD)) {
        state_of_vehicle = V_FORWARD_TURN_LEFT;
        current_speed = turn_speed;
    } else if ((left_motor_state == M_FORWARD) && (right_motor_state == M_BACKWARD)) {
        state_of_vehicle = V_FORWARD_TURN_RIGHT;
        current_speed = turn_speed;        
    } else {
        state_of_vehicle = V_STOPPED;
        current_speed = 0;
...
//----------------------------------------------------------------------------
// set_motor_speed_dir : configure a motor for speed and direction
// ===================
//
// Notes
...
//
// Parameters
//      motor_unit    : LEFT_MOTOR or RIGHT_MOTOR
//      int_pwm_width : 0% to 100%
//

void set_motor_speed_dir(motor_t motor_unit, int int_pwm_width) {

	if (int_pwm_width >= 0) {
		set_motor(motor_unit, M_FORWARD, int_pwm_width);
	} else {
		set_motor(motor_unit, M_BACKWARD, ((~int_pwm_width) + 1)); //2's complement
	}
}
...
//      state     : M_OFF, M_FORWARD, M_BACKWARD, or M_BRAKE
//      pwm_width : 0% to 100%
//
    ...
    if (unit == RIGHT_MOTOR) {
        switch (state) {
            case M_OFF :        // set FREEWHEEL condition
                ...
                left_motor_state = M_OFF;
                break;
            case M_FORWARD :    // set LOW on RM_PWM2 and pwm on LM_PWM1
                setReg16(TPM1C2V, period_count);      // set pwm on RM_PWM1
                setReg16(TPM1C3V, pulse_count);       // set LOW on RM_PWM2
                left_motor_state = M_FORWARD;
                break;
            case M_BACKWARD :   // set LOW on RM_PWM1 and RM_PWM2
...
void vehicle_stop(void) {
    ...
    set_motor(RIGHT_MOTOR, M_BRAKE, 0);
    set_motor(LEFT_MOTOR , M_BRAKE, 0);
}
...
//
    ...
    WAIT_SWITCH_RELEASED(switch_A);
    clr_LED(LED_A);
    set_motor(LEFT_MOTOR, M_FORWARD, 60);
    set_motor(RIGHT_MOTOR, M_FORWARD, 60);
    DelayMs(5000);
    set_motor(LEFT_MOTOR, M_OFF, 0);
    set_motor(RIGHT_MOTOR, M_OFF, 0);
//
...
//
    PROMPT_SWITCH_A;
    set_motor(LEFT_MOTOR, M_BACKWARD, 60);
    set_motor(RIGHT_MOTOR, M_FORWARD, 60);
    DelayMs(3000);
    set_motor(LEFT_MOTOR, M_OFF, 0);
    set_motor(RIGHT_MOTOR, M_OFF, 0);
...
//
// Initialise pseudo random number generator with value in 8mS tick counter.
// Will have a bit of entropy due to time when user presses button A.
//
    init_prng(tick_count_8, 0, 0);
    sys_mode = RUNNING;
...
//----------------------------------------------------------------------------
// set_log_pin : set pin 6 of PTFD I/O register
// ===========
//
// Used to generate signal for an oscilloscope
...
//----------------------------------------------------------------------------
// clear_log_pin : clear pin 6 of PTFD I/O register
// =============
//
