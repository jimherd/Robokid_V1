...
//
// run an one of PLAY/COLLECT/SAVE/RECALL/DUMP commands
// 
            ...
            if (switch_A == PRESSED) {            
                ...
                switch (activity) {
                    case CMD_FORWARD    :
                        ...
                        store_instruction(RAM_sequence.uint16, seq_ptr, PUSH_L8, IMMEDIATE, M_FORWARD); seq_ptr++;    
                    ...
                    case CMD_BACKWARD   :
                        ...
                        distance =  (distance * WHEEL_CONSTANT) / 100;    // convert to wheel counts
                        store_instruction(RAM_sequence.uint16, seq_ptr, PUSH_L8, IMMEDIATE, WHEEL_SENSOR_CALIBRATE_SPEED); seq_ptr++;
                        store_instruction(RAM_sequence.uint16, seq_ptr, PUSH_L8, IMMEDIATE, M_BACKWARD); seq_ptr++;    
                        store_instruction(RAM_sequence.uint16, seq_ptr, PUSH_L8, IMMEDIATE, LEFT_MOTOR); seq_ptr++;
                        store_instruction(RAM_sequence.uint16, seq_ptr, SET_PARAMETER, NO_MOD, SPEED); seq_ptr++;
                        store_instruction(RAM_sequence.uint16, seq_ptr, PUSH_L8, IMMEDIATE, WHEEL_SENSOR_CALIBRATE_SPEED); seq_ptr++;
                        store_instruction(RAM_sequence.uint16, seq_ptr, PUSH_L8, IMMEDIATE, M_BACKWARD); seq_ptr++;
    ...
    for (i=0 ; i < MAX_STRIP_CMDS ; i++) {
        ...
        switch (seq.strip_data[i][0]) {
            ...
            case STRIP_CMD_SPIN_LEFT :
                set_motor(LEFT_MOTOR, M_BACKWARD, STRIP_PLAY_SPIN_SPEED);
                set_motor(RIGHT_MOTOR, M_FORWARD, STRIP_PLAY_SPIN_SPEED);
                DelayMs(duration * 100);          
                break;
            case CMD_SPIN_RIGHT :
            case STRIP_CMD_SPIN_RIGHT :
                set_motor(LEFT_MOTOR, M_FORWARD, STRIP_PLAY_SPIN_SPEED);
                set_motor(RIGHT_MOTOR, M_BACKWARD, STRIP_PLAY_SPIN_SPEED);
                DelayMs(duration * 100);          
                break;
            case CMD_FORWARD :
            case STRIP_CMD_FORWARD :
                set_motor(LEFT_MOTOR, M_FORWARD, STRIP_PLAY_FORWARD_SPEED);
                set_motor(RIGHT_MOTOR, M_FORWARD, STRIP_PLAY_FORWARD_SPEED);
                DelayMs(duration * 100);            
                break;
            default :
                return  0;
