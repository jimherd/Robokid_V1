...
// 
        ...
        switch (activity) {
            case SIMPLE_TEST :
                do_timed_move(V_FORWARD, 20);
                do_timed_move(V_BACKWARD, 20); 
                do_timed_move(V_SPIN_LEFT, 10);
                do_timed_move(V_SPIN_RIGHT, 10);
                vehicle_stop();
                break;
            case RUN_BACKWARD :
                do_timed_move(V_BACKWARD, 20);                             
                break;
            case RUN_SPIN_RIGHT :
                do_timed_move(V_SPIN_RIGHT, 20);
                break;                                                                                  
            case RUN_SPIN_LEFT :
                do_timed_move(V_SPIN_LEFT, 20);
                break;      
            case RUN_TEST :
                run_test();
                break;
...
//----------------------------------------------------------------------------
// do_timed_move : implement a simple robot move
// =============
//
// Notes
//		"left_speed" and "right_speed" are GLOBAL variables
//

uint8_t do_timed_move(uint8_t direction, uint8_t time) 
{
	set_LED(LED_A, FLASH_ON); 
    ...
    DelayMs(time * 100);  // time in units of 100mS
    return 0;
}
...
//
            ...
            if ((now_time - start_time) > MOTOR_TEST_TIME_OUT) {
                set_motor(LEFT_MOTOR, M_BRAKE, left_speed);
                set_motor(RIGHT_MOTOR, M_BRAKE, right_speed);
                sys_error = TIME_OUT; 
                break;
            }
//
