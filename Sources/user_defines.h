...
//
// macros relevant to motors
//
#define SET_MOTORS(l_pwm, r_pwm)               \
	set_motor_speed_dir(LEFT_MOTOR,  (l_pwm)); \
	set_motor_speed_dir(RIGHT_MOTOR, (r_pwm));  
...
//
// Macros for log pin output
//
#define SET_LOG_PIN	    	{ asm bset  6,PTFD;}
...
//----------------------------------------------------------------------------
// system/vehicle/motor/sensor action/state definitions
//
// SYSTEM
...
typedef enum { V_STOPPED, V_FORWARD, V_BACKWARD, 
			   V_FORWARD_TURN_LEFT, V_FORWARD_TURN_RIGHT,
               V_BACKWARD_TURN_LEFT, V_BACKWARD_TURN_RIGHT, 
               V_SPIN_LEFT, V_SPIN_RIGHT } vehicle_state_t;

typedef enum  { S_INIT, S_PID, S_FIND_LINE_FROM_LOST, 
				S_FIND_LINE_FROM_LEFT, S_FIND_LINE_FROM_RIGHT, S_FOUND_L, S_FOUND_R } line_follow_state_t;

// MOTORS

typedef enum {LEFT_MOTOR, RIGHT_MOTOR} motor_t;
typedef enum {M_OFF, M_FORWARD, M_BACKWARD, M_BRAKE} motor_state_t;
typedef enum {FULL_SPEED=100, FAST=75, HALF_SPEED=50, SLOW=25} speed_t;

// IR SENSORS

enum line_sensors { LR_WW=0b00, LR_WB=0b01, LR_BW=0b10, LR_BB=0b11 };
