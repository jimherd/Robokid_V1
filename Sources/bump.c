...
//
        if (switch_B == PRESSED) {
            ...
            if (speed_differential > 0) {
                left_speed = LINE_BUMP_SPEED;
                right_speed = LINE_BUMP_SPEED - speed_differential;
            } else {
                speed_differential = -speed_differential;
                left_speed = LINE_BUMP_SPEED - speed_differential; 
                right_speed = LINE_BUMP_SPEED;
            }
...
uint8_t       ad_value, line_L, line_R, line;
uint16_t	  time_reverse, time_spin;
int8_t        speed_differential;
uint16_t      time; 
mode_state_t  state;
    ...
    state = MODE_INIT;
    left_speed = LINE_BUMP_SPEED;
    right_speed = LINE_BUMP_SPEED + SPEED_DIFFERENTIAL; // ADDED for B4
    time_reverse = DEFAULT_REVERSE_TIME;
    time_spin = DEFAULT_SPIN_TIME;
    speed_differential = SPEED_DIFFERENTIAL;   // ADDED for B4
    set_LED(LED_A, FLASH_ON);
    set_LED(LED_B, FLASH_ON);
    set_LED(LED_C, FLASH_ON);
    clr_LED(LED_D);
...
//
// get random reverse/spin times
//
		time_reverse = (get_random_byte_in_range(5,12) * 100);
		time_spin    = (get_random_byte_in_range(3, 7) * 100);       
//
// process 4 possible options for 2 line sensors 
//
        switch (line) {
        	case LR_WW : //  00 - no line detected therfore drive straight forward
                set_motor(LEFT_MOTOR, M_FORWARD, left_speed);
                set_motor(RIGHT_MOTOR, M_FORWARD, right_speed);           
                break;
                
			case LR_WB : //  01 - line detected by right sensor therfore turn left
                vehicle_stop();
//                SOUND_TAPE_BUMP; 
                set_motor(LEFT_MOTOR, M_BACKWARD, LINE_BUMP_SPEED);
                set_motor(RIGHT_MOTOR, M_BACKWARD, (LINE_BUMP_SPEED - SPEED_DIFFERENTIAL));
                DelayMs(time_reverse);                
                set_motor(LEFT_MOTOR, M_FORWARD, LINE_BUMP_SPEED);
                set_motor(RIGHT_MOTOR, M_BACKWARD, LINE_BUMP_SPEED); 
                DelayMs(time_spin);           
                break;

            case LR_BW : //  10 - line detected by left sensor therefore turn right
                vehicle_stop();
//                SOUND_TAPE_BUMP;
                set_motor(LEFT_MOTOR, M_BACKWARD, (LINE_BUMP_SPEED - SPEED_DIFFERENTIAL));
                set_motor(RIGHT_MOTOR, M_BACKWARD, LINE_BUMP_SPEED);
                DelayMs(time_reverse);
                set_motor(LEFT_MOTOR, M_FORWARD, LINE_BUMP_SPEED);
                set_motor(RIGHT_MOTOR, M_BACKWARD, LINE_BUMP_SPEED); 
            ...
            case LR_BB : //  11 - line detected by both sensors :: reverse + random spin
                vehicle_stop();
//                SOUND_TAPE_BUMP;
                set_motor(LEFT_MOTOR, M_BACKWARD, LINE_BUMP_SPEED);
                set_motor(RIGHT_MOTOR, M_BACKWARD, LINE_BUMP_SPEED);
                ...
                if (get_random_bit() == 0) {   // spin in a random direction                  
                    set_motor(LEFT_MOTOR, M_BACKWARD, LINE_BUMP_SPEED);
                    set_motor(RIGHT_MOTOR, M_FORWARD, LINE_BUMP_SPEED);
                }else {
                    set_motor(LEFT_MOTOR, M_FORWARD, LINE_BUMP_SPEED);
                    set_motor(RIGHT_MOTOR, M_BACKWARD, LINE_BUMP_SPEED);
                }
...
//                
                ...
                if (speed_value < 85 ) {
                    ...
                    if (speed_value > 170 ) {
                    left_speed = LINE_BUMP_SPEED - 20;
                    right_speed = LINE_BUMP_SPEED;
                    } else {
                        left_speed = LINE_BUMP_SPEED;
                        right_speed = LINE_BUMP_SPEED - 20;
                    }
                }     
                break;
