...
//                  Robokid
//----------------------------------------------------------------------------
// sketch.c : code to run sketch modes
// =========
//
// Description
//
...
//----------------------------------------------------------------------------
// run_sketch_mode : run one of a set of sketch activities
// ================
//
// Notes
//
...
//          switch C = exit to main selction level
//          switch D = step through selections
//
uint8_t run_sketch_mode(void) 
{
uint8_t   activity;

    D_MODE_LEDS;
    activity = FIRST_SKETCH_MODE;
    show_dual_chars(SKETCH_MODE_CODE, ('0'+ activity), (A_TO_FLASH | 10));
//
// main loop
//       
    FOREVER {
...
//
        if (switch_D == PRESSED) {
            ...
            if (activity > LAST_SKETCH_MODE) {
                activity = FIRST_SKETCH_MODE;
            }
            show_dual_chars('S', ('0'+ activity), (A_TO_FLASH | 10));
        }
...
// 
        show_dual_chars(SKETCH_MODE_CODE, ('0'+ activity), 0);
        push_LED_display();       
        switch (activity) {
                case SKETCH_MODE_0 :                          // in progress
                    run_sketch_mode_0();
                    break;
                case SKETCH_MODE_1 :                          // in progress
                    run_sketch_mode_1();
                    break;
                case SKETCH_MODE_2 :                          // in progress
                    run_sketch_mode_2();
                    break;                                        
                default :
                    break;
...
//----------------------------------------------------------------------------
// run_sketch_mode_0 : draw a spiral pattern
// ==================
//
// Description
...
//
        if (state == MODE_RUNNING) {   
            GET_TIMER16(time);
            if (time > SKETCH_MODE_0_TIME_OUT ) {
                state = MODE_INIT;
                vehicle_stop();
                continue;
            }
...
//        
        if (get_random_bit() == 1) {
            spiral_mode = LEFT_SPIRAL;
             set_motor(LEFT_MOTOR, M_BACKWARD, MAX_SPIRAL_SPEED);
             set_motor(RIGHT_MOTOR, M_FORWARD, MAX_SPIRAL_SPEED);        
        } else {
            spiral_mode = RIGHT_SPIRAL;
            set_motor(LEFT_MOTOR, M_FORWARD, MAX_SPIRAL_SPEED);
            set_motor(RIGHT_MOTOR, M_BACKWARD, MAX_SPIRAL_SPEED);
        }
//
// run spiral code
//
        for (i = MAX_SPIRAL_SPEED ; i > STOP_SPEED ; i--) {
            if (spiral_mode == LEFT_SPIRAL) {
                set_motor(LEFT_MOTOR, M_BACKWARD, i);
            } else {
                set_motor(RIGHT_MOTOR, M_BACKWARD, i);
            }
        ...
        for (i = STOP_SPEED ; i <=MAX_SPIRAL_SPEED ; i++) {
            if (spiral_mode == LEFT_SPIRAL) {
                set_motor(LEFT_MOTOR, M_FORWARD, i);
            } else {
                set_motor(RIGHT_MOTOR, M_FORWARD, i);
            }
            DelayMs(spiral_update_time);
            if (get_adc(FRONT_SENSOR_C) < SENSE_LOW ) {        // check for bump
                vehicle_stop ();
...
//----------------------------------------------------------------------------
// run_sketch_mode_1 : generate spirograph pattern
// ==================
//
// Description
...
//
    for (i = 0; i <= NOS_PATTTERN_LINES ; i++ ) {
        set_motor(LEFT_MOTOR, M_FORWARD, SPIROGRAPH_SPEED);
        set_motor(RIGHT_MOTOR, M_FORWARD, SPIROGRAPH_SPEED);
        DelayMs(line_draw_time * 50);
        vehicle_stop();
        DelayMs(10);
        set_motor(LEFT_MOTOR, M_FORWARD, SPIROGRAPH_SPEED);
        set_motor(RIGHT_MOTOR, M_BACKWARD, SPIROGRAPH_SPEED);
...
uint8_t run_sketch_mode_2(void) 
{
    return 0;
}
